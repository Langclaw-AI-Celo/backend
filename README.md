...
| Automation | `POST /api/automation/tasks`, `runs`, `settings`, `notifications`, `telegram/webhook`, `webhooks/:slug` |
| Proof | `POST /api/proofs/decisions`, `POST /api/proofs/readiness` |
| Strategy Lab | `POST /api/strategy/scan-pairs`, `backtest`, `paper-trade`, `runs` |
...
| --- | --- | --- |
| `LangclawUsageVault` | `npm run deploy:usage-vault -- --chain mantle\|celo --write-env` | `{MANTLE,CELO}_LANGCLAW_USAGE_VAULT_ADDRESS` |
| `LangclawRegistry` | `npm run deploy:registry -- --chain mantle\|celo --write-env` | `{MANTLE,CELO}_LANGCLAW_REGISTRY_ADDRESS` |
| `LangclawTradingJournal` | `npm run deploy:trading-journal -- --chain mantle\|celo --write-env` | `{MANTLE,CELO}_LANGCLAW_TRADING_JOURNAL_ADDRESS`, `{MANTLE,CELO}_TRADING_JOURNAL_ENABLED` |
