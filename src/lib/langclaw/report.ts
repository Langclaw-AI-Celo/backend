...
function buildSmartMoneyReport(
  ...
  const entities: ResearchReportEntity[] = rows.map((row, index) => ({
    ...
    summary: "Direct smart-money or holder-flow evidence from the tool output.",
    metrics: row.metrics,
    sourceIds: [],
    toolIds: [row.toolId],
  }));
  ...
  return {
    kind: "smart-money",
    title: `${input.plan.chainName} Smart Money Brief`,
    asOfUtc: input.generatedAt,
    executiveSummary: entities.length
      ? `This run returned direct smart-money evidence for ${input.plan.chainName}, with ${entities.length} ranked flow item(s) available for review.`
      : `This run did not return direct smart-money rows for ${input.plan.chainName}; the output stays narrative-first and ${localOnly ? "analysis-only" : "coverage-limited"}.`,
    bottomLine: entities.length
      ? "Use the ranked flow output as a research starting point, then confirm the strongest wallet or token flows with a second source."
      : "Treat this as directional smart-money research only until a direct provider returns usable rows.",
    ...
    sections: [
      {
        ...
        markdown: entities.length
          ? "Direct provider evidence returned structured smart-money or holder-flow rows."
          : localOnly
            ? "The available smart-money output stayed analysis-only because direct providers did not return structured rows."
            : "Direct smart-money evidence was incomplete, so this run stays narrative-first.",
        sourceIds: [],
      ...
      },
      {
        ...
        title: "Data Context",
        markdown:
          "Ranked smart-money entities are emitted only when the run includes direct provider rows. Local synthesis output can summarize evidence gaps, but it does not create ranked wallet tables by itself.",
        sourceIds: [],
        toolIds: input.tools.map((tool) => tool.commandId),
      },
      {
        ...
        title: "Conclusion",
        markdown: entities.length
          ? "The direct provider rows are usable, but they still need manual confirmation before being framed as verified accumulation."
          : "The run is informative but not conclusive. Confirm wallet or holder flow with a second on-chain source before escalating the claim.",
        sourceIds: [],
        toolIds: input.tools.map((tool) => tool.commandId),
      },
    ],
...
function deriveOnChainConfidence(
  ...
  const directSuccesses = tools.filter(
    (tool) => tool.provider !== "local" && tool.status === "success"
  ).length;
  const directFailures = tools.filter(
    (tool) => tool.provider !== "local" && tool.status === "failed"
  ).length;
...
function buildOnChainReportCaveats(
  ...
  const notes = uniqueStrings([
    input.caveat,
    ...input.tools
      .filter((tool) => tool.status === "failed" && tool.provider !== "local")
      .map(
        (tool) =>
          `${providerLabel(tool.provider)} failed (${normalizeSentence(tool.error || tool.summary)}).`
      ),
  ]);
...
function buildWorkflowCaveats(input: BuildWorkflowResearchReportInput) {
  ...
  const notes = [
    ...
    ...(input.providerTrace ?? [])
      ...
      .map(
        (entry) =>
          `${providerLabel(entry.provider)} failed (${normalizeSentence(entry.message)}).`
      ),
    sectionalCaveat,
...
function buildWorkflowRecommendation(
  ...
  if (kind === "smart-money") {
    return "Treat this as directional smart-money research and confirm wallet or holder flow with a second on-chain source.";
  }
...
function buildWorkflowBottomLine(
  ...
  if (kind === "smart-money") {
    return directMetricsAvailable
      ? "Social and on-chain evidence can support a smart-money watchlist, but the claim still needs direct manual confirmation."
      : "Social momentum may be visible, but verified smart-money confirmation is still incomplete.";
  }
...
function summarizeToolResults(tools: OnChainToolResult[]) {
  ...
  if (!successful.length) {
    return "No direct provider returned usable evidence in this run.";
  }

  return `The run returned ${successful.length} successful tool result(s) and ${failed.length} failed tool result(s).`;
}
...
function normalizeMetrics(record: Record<string, unknown>) {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (typeof value === "number") {
      ...
      const parsed = Number(value);
      metrics[key] = Number.isFinite(parsed) && value.trim() !== ""
        ? roundNumber(parsed)
        : value;
    } else if (value == null) {
      metrics[key] = null;
